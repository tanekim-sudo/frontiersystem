 ...
 * Mirrors the Python Chicago Fed collector logic.
 */
...
export function parseChicagoFedExcel(content) {
  ...
  /** Weekly history from the same spreadsheet (backfill = re-download xlsx; no separate API). */
  const timeseries = merged
    .map((row) => ({
      date: row._d,
      forecast_unemployment: num(row.forecast50f ?? row.forecast50a),
      layoffs_separations_rate: num(row.layoffs_other_seps),
      hiring_rate_unemployed: num(row.hiring_rate_uw),
      official_u3: num(row.official_u3),
    }))
    .filter((r) => r.date);
