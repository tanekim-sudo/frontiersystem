...
export const SERIES_MAP = {
  ...
  LNS11300000: { name: "Labor Force Participation", category: "labor" },
...
/** Full history for charts (parallel). Backfill depth controlled by observationStart + limit. */
export async function fetchAllFredHistories(apiKey, { observationStart = "2015-01-01", limit = 320 } = {}) {
  ...
  const settled = await Promise.allSettled(
    ids.map((id) => fetchFredSeriesAscending(id, apiKey, observationStart, limit)),
  );
  const out = {};
  ids.forEach((id, i) => {
    const r = settled[i];
